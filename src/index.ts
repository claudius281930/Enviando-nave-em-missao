...
function addCrewMember(member: string, spaceship: {
    ...
    if (spaceship.crew.length >= spaceship.crewLimit) {
        alert(`${member} não pode ser adicionado. Limite atingido para a nava ${spaceship.name}`)
    } else {
        spaceship.crew.push(member)
        alert(`${member} foi adicionado a tripulação da nava ${spaceship.name} com sucesso!`)
    }
}
...
function firstMenuOption() {
    ...
    const confirmation = confirm(`confirmao registro da nava ${name}\nPiloto: ${pilot}\nTamanho da Tripulação: ${crewLimit}`)
